...
const CAMPAIGN_CARDS = [
  { card: 'Black 2',    id: 'aircraft-shortage' },
  { card: 'Black 3',    id: 'coordinated-strikes' },
  { card: 'Black 4',    id: 'daring-commander' },
  { card: 'Black 5',    id: 'enemy-codes' },
  { card: 'Black 6',    id: 'enemy-plans' },
  { card: 'Black 7',    id: 'escort-carriers' },
  { card: 'Black 8',    id: 'extended-craft-range' },
  { card: 'Black 9',    id: 'false-codes' },
  { card: 'Black 10',   id: 'improved-armour', classPick: true },
  { card: 'Black Jack', id: 'increased-aircraft', classPick: true },
  { card: 'Black Queen',id: 'ineffective-aa' },
  { card: 'Black King', id: 'long-range-radar' },
  { card: 'Black Ace',  wild: true },
  { card: 'Red 2',      id: 'mobile-force-doctrine' },
  { card: 'Red 3',      id: 'near-range-radar' },
  { card: 'Red 4',      id: 'overconfident-commander' },
  { card: 'Red 5',      id: 'overcomplicated-plans' },
  { card: 'Red 6',      id: 'overwhelming-air-patrols' },
  { card: 'Red 7',      id: 'rookie-pilots' },
  { card: 'Red 8',      id: 'seasoned-pilots' },
  { card: 'Red 9',      id: 'superior-fighters' },
  { card: 'Red 10',     id: 'torpedo-belts', classPick: true },
  { card: 'Red Jack',   id: 'unrefined-fuel' },
  { card: 'Red Queen',  id: 'unreliable-torpedoes' },
  { card: 'Red King',   id: 'wily-commander' },
  { card: 'Red Ace',    wild: true },
];
...
const Icon = {
  ...
  Strike: (p) => <svg width="22" height="22" viewBox="0 0 512 512" aria-hidden="true" fill="currentColor" stroke="none" {...p}>
    <path fill="currentColor" d="M288.625 19.625C252.95 45.54 233.16 81.95 226.655 126.938l41.095 23.687c9.348-5.82 17.33-12.415 24.22-19.656c4.383 22.757 20.633 45.578 39.874 56.686l65.5 37.813c24.465 14.124 54.433 9.697 73.812-8.907l7.97 4.593l17.312-30l-7.938-4.594c6.426-26.087-4.752-54.28-29.22-68.406l-65.5-37.812c-19.76-11.41-48.63-13.89-70.78-5.53c2.947-10.077 5.108-20.605 6.688-31.47l-41.063-23.72zm12.313 174.97c-49.957 19.17-84.558 55.62-106.313 106.31l40.688 40.72c12.79-3.888 24.24-9.11 34.593-15.438c-2 28.05 9.847 59.878 28.906 78.938l64.907 64.906c24.242 24.245 60.782 28.477 89.342 12.75l7.875 7.876l29.72-29.72l-7.844-7.873c15.736-28.564 11.465-65.097-12.782-89.344l-64.905-64.908c-19.58-19.58-52.628-31.56-81.22-28.718c6.62-10.884 12.457-22.543 17.72-34.78l-40.688-40.72zm-182.157.655c-35.453-.015-68.357 12.318-99 35.22l12.72 47.468c11.113 2.584 21.805 3.602 32.156 3.343c-13.452 19.88-18.267 48.465-12.312 70.69l20.28 75.686c7.576 28.27 32.77 46.972 60.595 47.53l2.468 9.19l34.656-9.282l-2.47-9.188c23.81-14.4 36.262-43.205 28.69-71.47l-20.25-75.686c-6.117-22.822-25.445-45.77-47.783-55.875c9.534-5.22 18.83-11.36 27.94-18.156l-12.72-47.44c-8.458-1.34-16.787-2.027-24.97-2.03"/>
  </svg>,
    ...
    <tr className={'unit-row ' + (overCapacity ? 'over-capacity' : '')}>
      ...
      <td className="col-role">
        <div className="role-cell">
          <span>{cls.role || ''}</span>
          {(() => { const sp = effectiveSpecial(cls, fleet); return sp ? <span className="special" data-tip={specialTip(sp)}>{sp}</span> : null; })()}
        </div>
      </td>
        ...
        <div className="armour-modal-body">
          {classes.map(c => (
            <button type="button" key={c.id}
              className={'armour-opt' + (current === c.id ? ' active' : '')}
              onClick={() => { onPick(c.id); onClose(); }}>
              <span className="cls-tag" data-sprite={c.sprite}>{c.sprite}</span>
              <span className="armour-opt-name">{c.name}</span>
              <span className="armour-opt-cur">{c.special && c.special.match(/Armoured \((\d+)\)/) ? c.special.match(/Armoured \(\d+\)/)[0] : 'No armour'}</span>
            </button>
          ))}
        </div>
      </div>
...
const MEANINGS = {
  ...
  'Taiyo': 'Great Hawk',
  'Unyo': 'Cloud Hawk',
  'Chuyo': 'Hawk of the Open Sea',
  'Kaiyo': 'Sea Hawk',
  'Shinyo': 'Divine Hawk',
  'Hosho': 'Soaring Phoenix',
  'Ryuho': 'Dragon Phoenix',
  'Furutaka': 'Old Hawk (Mt. Furutaka)',
  'Kako': 'Kako River',
  'Aoba': 'Green Leaves',
  'Kinugasa': 'Silk Parasol (Mt. Kinugasa)',
  'Kuma': 'Kuma River',
  'Tama': 'Tama River',
  'Kitakami': 'Kitakami River',
  'Oi': 'Oi River',
  'Kiso': 'Kiso River',
  'Yubari': 'Yubari River',
  'Katori': 'Fragrance Gathering (Katori Shrine)',
  'Kashima': 'Deer Island (Kashima Shrine)',
  'Isuzu': 'Fifty Bells',
  'Yahagi': 'Arrow-Fletcher River',
  'Sakawa': 'Sakawa River',
  'Nisshin': 'Daily Advance',
  'Akitsushima': 'Dragonfly Islands (ancient name for Japan)',
  'Minekaze-class': 'Peak Wind',
  'Kamikaze-class': 'Divine Wind',
  'Shimakaze-class': 'Island Wind',
  'Reisen': 'Zero Fighter (Type 0)',
  'Shiden': 'Violet Lightning',
  'Shiden-Kai': 'Violet Lightning, Improved',
